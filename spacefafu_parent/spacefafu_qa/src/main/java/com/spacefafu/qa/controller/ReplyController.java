...
import com.spacefafu.qa.service.ReplyService;
...
public class ReplyController {
	...
	public Result findSearch(@RequestBody Map searchMap , @PathVariable int page, @PathVariable int size){
		...
		List<Reply> replyList = pageList.getContent();
		for (Reply reply:replyList) {
			Map<String,String> map=new HashMap<>();
			map.put("parentid",reply.getId());
			Page<Reply> childs = replyService.getChilds(map, 1, 20);
			if(childs.getContent()!=null){
				reply.setChildReplys(childs.getContent());
			}
		}
...
//		reply1.setContent("222");
//		replies.add(reply);
//		replies.add(reply1);
//		pageList.getContent().get(0).setChildReplys(replies);
